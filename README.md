...
Context: a proposed experiment on cross-language, LLM-mediated discussion of an IETF draft (see draft-fengfar-led, "Dealing with LLMs in IETF Discussions"). The draft's authors asked for a design, an evaluation, and a check of the state of the art. This repository is the check. Raw reports with full citations: [sweep/](sweep/) (three subagent reports plus the retrieval manifest). Retrieved copies: [papers/](papers/); each entry below names its file. Summaries below are in short technical English. UNVERIFIED means the subagent did not confirm the source against a primary page. Order: 2024 and later first, then 2023 and earlier.

## Short answer

Nobody has done this experiment. Related studies exist. They agree on three points. Readers cannot see a fluent error. A text can feel clear and still transfer little. An AI label lowers trust by itself. Nobody has measured the unaided receiver in a real technical discussion.
...
- **Reader perception shifts on disclosure (arXiv 2510.24011, 2025).** Disclosure of AI authorship changed stated quality. It did not change comprehension in the same way. Method: pre and post disclosure comparison. [file: 2025-2510.24011-disclosure-perception.pdf]
...
- **AI-label RCT, N=164 (PMC11176609, 2024).** The same text labelled "ChatGPT" lost on competence (d=0.95), quality (d=0.39) and willingness to use (d=0.57). Topic did not change the effect. Method: between-subjects label swap. [file: 2024-pmc11176609-ai-label-rct.md]
...
- **LAAC: LLM as a Communicator (arXiv 2511.04184, 2025).** The LLM structures sender intent before the receiver sees it. Not cross-lingual. Adjacent only. [file: 2025-2511.04184-laac-communicator.pdf]
- **Time to Talk, LLM agents in Mafia games (arXiv 2506.05309, 2025).** Agents play as participants, not as mediators. Low relevance. [file: 2025-2506.05309-time-to-talk.pdf]

### Communities and institutions
- **draft-fengfar-led-01 (IETF, 2026-08-06).** Two voices: a skeptical reader and an AI-using non-native author. Names translation as a benefit. Warns that speed may reduce sender comprehension and reader trust. Proposes guidelines, not an experiment. [file: 2026-ietf-draft-fengfar-led-01.txt]
...
- **Wikimedia "Simple Article Summaries" backlash (2025, press).** Editors rejected the trial at once. Case study, not research. [file: 2025-wikimedia-simple-summaries-backlash.html]
...
- **Zhang, Owusu, Carpuat, Gao, CSCW 2022.** MT-translated subgroup logs shared before a meeting improved the meeting. MT worked as async context, not live speech. Method: 20 participants in quartets. [file: 2022-2209.02906-zhang-cscw.pdf]
...
- No study isolates the unaided receiver as the measured party.
- No symmetric design where both directions are LLM-mediated.
- No sealed-original, reveal-at-end protocol in any communication study found.
- Most AI-mediated-communication studies measure trust and perception. Few measure receiver comprehension.
- No peer-reviewed study of standards-body or mailing-list reaction to AI prose.
- No source frames the cost as moved from sender to receiver.
...
Every reference without a retrieved copy in llm-readability-papers/. Reason in brackets. The two phantom entries stay listed so the sweep's own gap is visible; do not cite them.

- **LLM plain-language summaries, crowdsourced study (PubMed 42044823, 2025).** Readers rated LLM and human summaries as equally clear. Readers answered more comprehension questions correctly after the human summaries. Method: comprehension questions plus clarity ratings. [not retrieved: PubMed wall, not open access]
...
- **Why ChatGPT "delves", COLING 2025.** The overuse of "delve" traces to RLHF labeler demographics. It is a pipeline artifact, not an AI property. Method: frequency analysis. [not retrieved: paper PDF not found, only FSU press coverage saved]
...
- **SHA-256 pre-registration blog (dev.to, 2026, not peer reviewed).** One author hashes a hypothesis before LLM trials. Existence proof only. [not retrieved: no source found in the sweep, likely phantom]
...
- **Conversational Grounding in MT-Mediated Communication (Language Grid volume, Springer 2011).** Success depends on the repair process, not raw MT accuracy. People add redundancy and confirmation checks. Partly UNVERIFIED, paywalled. [not retrieved: Springer book chapter, paywall]
...
- **Yamashita and Ishida, CSCW 2006.** MT changes how people collaborate, not only what they receive. Partners cannot shorten shared terms over a dialogue. Method: task-based experiment. Primary PDF not fetched. [not retrieved: no URL found in the sweep, primary PDF never fetched]
